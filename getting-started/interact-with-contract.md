...
RES=$(wasmcli tx wasm store contract.wasm --from fred \
    --gas-prices="0.025ucosm" --gas="auto" --gas-adjustment="1.2" -y)
...
wasmcli tx wasm execute $CONTRACT "$APPROVE" \
    --from thief \
    --gas-prices="0.025ucosm" --gas="auto" --gas-adjustment="1.2" -y
...
hitFaucet(defaultFaucetUrl, fredAddr, "ucosm")
fredClient.getAccount();

thiefClient.getAccount();
// if "undefined", do the following
hitFaucet(defaultFaucetUrl, thiefAddr, "ucosm")
thiefClient.getAccount();
