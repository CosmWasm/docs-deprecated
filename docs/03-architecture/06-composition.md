...
interfaces that should work consistently with all CosmWasm chains. The most basic one is to the `Bank` module, which
...
type to be include in the `Custom` variants, and it just needs to be agreed upon by the Cosmos SDK app (in Golang) on
...
in the native format (be it json, amino or protobuf), and the format of native messages change, then the contracts would
