...
An example of an actor is the Seller in the KFC restaurant. First
thing you do is you order your BSmart, so you are requesting an
action from him. So as from the system user, you can think
about this about the task as "sell and prepare my meal", but the
...
to be prepared by other actors, probably chefs. Then when chef
...
Obviously, the above-described workflow is kind of simplified. In
...
being chefs dispatcher which would collect all orders from
...
He also remembers how many actions he scheduled to every order id.
...
for client), or seller is just hungry. It is actually possible,
that no only Seller is eligible to call chef for food - possibly
...
how many actions he scheduled to chefs are not yet finished - this
number is actually part of Sellers state.

To give more realistic example of contract state, let's think about more
...
if I cannot check how much of it I actually own? It is a very good
question, and the answer to that is simple - the whole state of every
contract in our system is actually public. It is not universal for
...
However, before decreasing it, we obviously need to check if a sender
...
I didn't yet cover. As I said, whole state of every contract is public
...
This is where queries come into play. Queries are type of messages to
...
So we touched many things here, and I know it may be kind of confusing.
...
which fulfilled the quest (as we wanted it to be), but the list of players
...
never ends up in improper state). It is probably a bit complicated for now,
...
applied to the blockchain only if everything succeed, but in case that
